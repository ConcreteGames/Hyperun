...
#include "AdvancedSteamAchievementClearCallbackProxy.generated.h"

DECLARE_DYNAMIC_MULTICAST_DELEGATE_OneParam(FSteamAchievementsClearResult, EAdvancedSteamErrorCode, ErrorCode);
...
class UAdvancedSteamAchievementClearCallbackProxy : public UOnlineBlueprintCallProxyBase
{
	...
	FSteamAchievementsClearResult OnSuccess;

	// Called when there is an unsuccessful leaderboard query
	UPROPERTY(BlueprintAssignable)
	FSteamAchievementsClearResult OnFailure;

	// Queries a leaderboard for an integer value
	UFUNCTION(BlueprintCallable, meta = (BlueprintInternalUseOnly = "true", DisplayName="Clear SteamAchievement"), Category="Online|SteamAchievements")
	static UAdvancedSteamAchievementClearCallbackProxy* CreateProxyObjectForAchievementsClear(FName AchievementId);
...
private:
	...
	void OnAchievementsClear(bool bWasSuccessful, const EAdvancedSteamErrorCode& InErrorCode);
