...
class USteamUErBPLibrary : public UBlueprintFunctionLibrary
{
	GENERATED_UCLASS_BODY()
		...
		~USteamUErBPLibrary()
		{
			...
			/* // Deleting these will crash the game, since apparently they're... some kind of shared pointer, maybe? idk
			   // It's not very tidy not cleaning up after ourselves, but if the other option is to have a crash report screen every time the game closes...
			   // I mean, it's not like we can just hack it to show 'Salt the fries' these days
			UE_LOG(LogTemp, Log, TEXT("SteamUEr: Like..."));
			if (myStats != NULL) 
			{
				UE_LOG(LogTemp, Log, TEXT("SteamUEr: ... tears..."));
				delete myStats;
			}
			if (myUser != NULL) 
			{
				UE_LOG(LogTemp, Log, TEXT("SteamUEr: ... in..."));
				delete myUser;
			}
			UE_LOG(LogTemp, Log, TEXT("SteamUEr: ... the rain..."));
			*/
		}
